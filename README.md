...
A GitHub-aware simple task management app built with Blazor WebAssembly and backed by Firebase.

## Security Notes
Runs entirely client side in the browser and each user provides their own Firebase project ("bring your own Firebase"), so there is no shared backend. The entire database belongs to the user.
